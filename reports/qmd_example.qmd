...
author: "Tiffany Timbers & Jordan Bourak & Chuxuan Zhou"
execute:
  echo: false
format: 
    html:
...
## Results
![Horse populations for all provinces in Canada from 1906 - 1972.](../results/horse_pops_plot.png){#fig-1 width=65%}
...
We present the standard deviations in @tbl-1.


```{r}
#| warning: false
#| label: tbl-1
#| tbl-cap: Standard deviation of historical horse populations for each province
...
Additionally, note that in @tbl-1 we consider the sample standard deviation of the number of horses during the same time span as @fig-1.

![Horse populations for the province with the largest standard deviation](../results/horse_pop_plot_largest_sd.png){#fig-2 width=65%}


In @fig-2 we zoom in and look at the province of `{r} largest_sd`, which had the largest spread of values in terms of standard deviation.
